...
# -*- coding: utf-8 -*-
"""
Commands related to links and references.
...
def getCurrentScopeRegion(view, pt):
    ...
    p = pt
    while p > 0 and view.scope_name(p - 1) == scope:
        p -= 1
    r = pt
    while r < view.size() and view.scope_name(r) == scope:
        r += 1
    return sublime.Region(p, r)
    ...
    if definition_scope_name in scope or footnote_scope_name in scope:
        ...
        if refname_scope_name in scope:
            ...
            while pos >= 0 and view.substr(sublime.Region(pos, pos + 1)) in "[]":
                pos -= 1
            if not (
                    marker_scope_name in scope or marker_image_scope_name in scope or marker_literal_scope_name in scope):
                return (False, None, None)
            marker_text_end = findScopeFrom(view, pos, marker_text_end_scope_name) + 1
            if hasScope(view.scope_name(marker_text_end), refname_start_scope_name) and not hasScope(
                    view.scope_name(marker_text_end + 1), marker_end_scope_name):
                # of [Text][name] struct
                ...
                if defname_key in markers:
                    for marker in markers[defname_key].regions:
                        if defname[0] == "^":
                            ...
                            p = findScopeFrom(view, marker.begin(), marker_begin_scope_name, True)
                            if p > 0 and view.substr(sublime.Region(p - 1, p)) == "!":
                                edit_regions.append(sublime.Region(p - 1, p + 1))
                            else:
                                edit_regions.append(sublime.Region(p, p + 1))
                            if hasScope(view.scope_name(marker.end()), marker_text_end_scope_name):
                                if view.substr(sublime.Region(marker.end() + 1,
                                                              marker.end() + 2)) == "[":
                                    # [Text][]
                                    r = findScopeFrom(
                                    ...
                                    # [Text]
                                    edit_regions.append(sublime.Region(
    ...
    def run(self, edit):
        ...
        marker_order = sorted(
            ...
            key=lambda marker: min(
                markers[marker].regions,
                key=lambda reg: reg.a).a)
        ...
        flatrefs.sort(key=lambda x: marker_order[x[0].lower()]
                      if x[0].lower() in marker_order else 9999)
        ...
        output += "\n".join(("[%s] is referenced %s" % (markers[m].label,
                                                        get_times_string(len(markers[m].regions)))) for m in markers)
